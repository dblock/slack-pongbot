  ...
  if (hook) {
    ...
    case 'register':
      if (params.length != 2) {
        ...
        pong.findAndUpdatePlayer(hook.user_id, hook.user_name, function (err, user) {
          if (user) {
            res.json({ text: "You've already registered!" });
          } else {
            pong.registerPlayer(hook.user_id, hook.user_name, function (err, user) {
    ...
    case 'new_season':
      if (!process.env.ADMIN_SECRET) {
        ...
        pong.resetAll(function (err, user) {
          if (err) {
            res.json({ text: err.message });
          } else {
