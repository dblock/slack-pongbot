...
var pong = {
  ...
  registerPlayer: function (user_name, _) {
    new Player({
      user_name: user_name,
      wins: 0,
      losses: 0,
  ...
  findPlayer: function (user_name, _) {
  ...
  updateWins: function (user_name, _) {
  ...
  updateLosses: function (user_name, _) {
    pong.findPlayer(user_name, function (err, user) {
      if (user) {
        user.losses++;
        user.save(function (err, user) {
  ...
  createSingleChallenge: function (challenger_name, challenged_name, cb) {
    pong.ensureUniquePlayers([challenger_name, challenged_name], function (err) {
      if (err) return cb(err, null);
      pong.findChallenge(challenger_name, function (err, challenge) {
        if (challenge || err) {
          cb(err || new Error("There's already an active challenge between " + challenge.challenger.join(' and ') + ' and ' + challenge.challenged.join(' and ') + '.'), null);
        } else {
  ...
  createDoubleChallenge: function (c1, c2, c3, c4, cb) {
    pong.ensureUniquePlayers([c1, c2, c3, c4], function (err) {
      if (err) return cb(err, null);
  ...
  findChallenge: function (user_name, _) {
    pong.findPlayer(user_name, function (err, user) {
      if (user) {
        Challenge.findOne({ _id: user.currentChallenge }, function (err, challenge) {
          if (err) throw err;
          _(null, challenge);
        });
      } else {
        throw (err || new Error("User '" + user_name + "' does not exist."));
      }
    });
  },

  setChallenge: function (user_name, id, _) {
    pong.findPlayer(user_name, function (err, user) {
      if (user) {
  ...
  removeChallenge: function (user_name, _) {
    pong.findPlayer(user_name, function (err, user) {
      if (user) {
        if (user.currentChallenge) {
          ...
          throw new Error("User '" + user_name + "' has not been challenged.");
        }
      } else {
        throw(err || new Error("User '" + user_name + "' does not exist."));
      }
  ...
  acceptChallenge: function (user_name, _) {
    pong.findChallenge(user_name, function (err, challenge) {
      if (err || !challenge) {
        throw(err || new Error("No challenge to accept."), null);
      } else if (challenge.state == 'Proposed') {
        challenge.state = 'Accepted';
        challenge.save(function (err) {
          if (err) throw err;
          // TODO: reimplement who serves first
          _(new Error(user_name + ' accepted ' + challenge.challenger[0] + "'s challenge."), challenge);
        });
      } else if (challenge.state == 'Accepted') {
        throw new Error('You have already accepted ' + challenge.challenger[0] + "'s challenge.");
      } else {
        throw new Error("No challenge to accept.");
      }
    });
  },

  declineChallenge: function (user_name, _) {
    pong.findChallenge(user_name, function (err, challenge) {
      if (err || !challenge) {
        throw(err || new Error("No challenge to decline."), null);
        ...
        challenge.save(function (err) {
          ...
          Player.update({ currentChallenge: challenge._id }, { currentChallenge: null }, { multi: true }, function (err) {
            if (err) throw err;
            _(new Error(user_name + ' declined ' + challenge.challenger[0] + "'s challenge."), challenge);
          });
        });
      } else {
        throw new Error("No challenge to decline.");
      }
    });
  },

  calculateTeamElo: function (p1, p2, _) {
    pong.findPlayer(p1, function (err, user1) {
      pong.findPlayer(p2, function (err, user2) {
        _(null, (user1.elo + user2.elo) / 2);
      });
    });
  },

  eloSinglesChange: function (winner_name, loser_name, _) {
                  ...
                  pong.updateWins(winners[1], function (err) {
                    ...
                    pong.updateLosses(losers[0], function (err) {
                      ...
                      pong.updateLosses(losers[1], function (err) {
                        cb(new Error('Match has been recorded, ' + winners.join(' and ') + ' have defeated ' + losers.join(' and ') + '.'));
                      });
                    });
                  });
                });
              ...
              pong.eloSinglesChange(winner, loser, function(err) {
                ...
                pong.updateWins(winner, function (err) {
                  ...
                  pong.updateLosses(loser, function (err) {
                    if (err) return cb(err, challenge);
                    cb(new Error('Match has been recorded, ' + winner + ' has defeated ' + loser + '.'));
                  });
                });
              });
            }
  ...
  ensureUniquePlayers: function(players, _) {
    ...
    Object.keys(playerCounts).forEach(function (key, index, array) {
      var count = playerCounts[key];
      if (count > 1) {
        throw new Error('Does ' + key + ' have ' + 2 * count + ' hands?');
      }
    });
