...
var pong = {
  ...
  registerPlayer: function (user_id, user_name, cb) {
    new Player({
      user_id: user_id,
      user_name: user_name,
      wins: 0,
  ...
  findAndUpdatePlayer: function (user_id, user_name, cb) {
    Player.where({
      "$or" : [
        { user_name : user_name },
        { user_id : user_id }
      ...
      if (user) {
        if ((user.user_id != user_id) || (user.user_name != user_name)) {
  ...
  createDoubleChallenge: function (c1, c2, c3, c4, cb) {
    pong.findChallenge(c1, function (err, challenge) {
      if (challenge || err) {
        cb(err || new Error("There's already an active challenge between " + challenge.challengerString() + ' and ' + challenge.challengedString() + '.'), null);
      } else {
